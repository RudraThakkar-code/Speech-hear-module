...
-- Document Enums
CREATE TYPE document_classification AS ENUM ('MEDICAL_REPORT', 'AUDIO_REPORT', 'AUDIOLOGY_REPORT', 'CONSENT_FORM', 'CLINICAL_NOTES', 'OTHER');

-- History Enums
CREATE TYPE delivery_type AS ENUM ('VAGINAL', 'CESAREAN', 'INSTRUMENTAL', 'UNKNOWN');
CREATE TYPE prematurity_status AS ENUM ('FULL_TERM', 'PRETERM', 'UNKNOWN');
CREATE TYPE motor_milestones_status AS ENUM ('TYPICAL', 'DELAYED', 'UNKNOWN');
...
CREATE TABLE birth_history (
    ...
    prematurity_status prematurity_status NOT NULL,
...
CREATE TABLE developmental_history (
    ...
    motor_milestones_status motor_milestones_status NOT NULL,
...
CREATE TABLE speech_sample (
    ...
    CONSTRAINT chk_analysis_ineligibility CHECK (
        (analysis_eligibility = 'NOT_ELIGIBLE' AND analysis_ineligibility_reason IS NOT NULL)
        OR
        (analysis_eligibility <> 'NOT_ELIGIBLE' AND analysis_ineligibility_reason IS NULL)
    )
);
...
CREATE TABLE supervisor_review (
    ...
    CONSTRAINT chk_review_status CHECK (
        ...
        OR
        (review_status = 'COMPLETED' AND reviewed_at IS NOT NULL)
    )
);
...
CREATE TABLE ai_artifact (
    ...
    CONSTRAINT chk_ai_human_review CHECK (
...
## 20. Doctor Portal & Clinical Collaboration (Phase 18)
```sql
